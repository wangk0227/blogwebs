<template>
  ...
  <div>
    ...
    <div class="clearfix">
      <Button class="add_btn" @click="saveData" type="success" v-if="is_show">点击保存</Button>
      <Button class="add_btn" type="success" disabled v-else>点击保存</Button>
    </div>
  </div>
</template>
...
import {create_articles} from '../../requestapi/articles-api'
...
export default {
  ...
  methods: {
    saveData() {
      ...
      create_articles(this.formData).then(res => {
    ...
    resetDate() {
      ...
      this.formData.article_category = ''
      this.formData.recommend_state = '0'
      this.formData.order = 1
      this.$refs.getData.clearEditor()
    }
  },
  created() {
    this.getCategory()
